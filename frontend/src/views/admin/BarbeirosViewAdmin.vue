<template>
  <div class="barbeiros-admin">
    ...
    <div class="header-actions">
      <h2 class="title">
        Gerenciamento de <span class="gold">Barbeiros</span>
      </h2>
      <Button
        label="Novo Barbeiro"
        icon="pi pi-plus"
    ...
    >
      ...
      <Column header="Especialidades">
        <template #body="slotProps">
          <div class="tags-container">
            <Tag
              v-for="esp in slotProps.data.especialidades"
              :key="esp"
              :value="formatEnum(esp)"
              severity="secondary"
              class="custom-tag"
            />
          </div>
      ...
      <Column header="Status">
        <template #body="slotProps">
          <Tag
            :value="slotProps.data.estaAtivo ? 'Ativo' : 'Inativo'"
    ...
    <Dialog
      v-model:visible="barbeiroDialog"
      :header="barbeiro.id ? 'Editar Barbeiro' : 'Novo Barbeiro'"
      :modal="true"
    ...
    >
      ...
      <div class="field">
        ...
        />
        <small v-if="barbeiro.fotoPath" class="text-gold"
          >Já possui foto salva.</small
        >
      </div>
      ...
      <div class="field">
        ...
        <MultiSelect
          v-model="barbeiro.especialidades"
      ...
      <div class="horarios-section">
        ...
        <div
          v-for="(horario, index) in barbeiro.horariosTrabalho"
          :key="index"
          class="horario-row"
        >
          <Dropdown
            v-model="horario.diaDaSemana"
        ...
        <Button
          ...
          icon="pi pi-plus"
          class="p-button-text p-button-sm gold-text"
          @click="addHorario"
        />
      </div>
...
const loadBarbeiros = async () => {
  ...
  try {
    ...
    toast.add({
      severity: "error",
      summary: "Erro",
      detail: "Falha ao carregar lista",
      life: 3000,
    });
  } finally {
    loading.value = false;
...
const editBarbeiro = (data: Barbeiro) => {
  barbeiro.value = {
    ...data,
    horariosTrabalho: data.horariosTrabalho.map((h) => ({ ...h })),
  };
  selectedFile.value = null;
  barbeiroDialog.value = true;
};
...
const addHorario = () => {
  ...
  barbeiro.value.horariosTrabalho.push({
    diaDaSemana: "",
    horaInicio: "",
    horaFim: "",
  });
...
const saveBarbeiro = async () => {
  ...
  try {
    const input: BarbeiroInput = {
      ...
      horariosTrabalho:
        barbeiro.value.horariosTrabalho?.map(({ id, ...h }) => h) || [],
